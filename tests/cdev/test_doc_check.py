...
def test_an_unlisted_file_is_reported(tmp_path):
    ...
    assert doc_check.check(root) == ["src/ARCHITECTURE.md: src/retry.c is not listed"]


def test_a_listed_file_that_is_gone_is_reported(tmp_path):
    root = repo(tmp_path, files=("hal_uart.c",))
    assert doc_check.check(root) == ["src/ARCHITECTURE.md: lists main.c, which is not in src/"]
...
def test_a_document_without_a_files_section_is_reported(tmp_path):
    ...
    assert doc_check.check(root) == ["src/ARCHITECTURE.md: has no ## Files section"]
...
def test_ignored_build_output_is_skipped_in_git(tmp_path):
    ...
    assert doc_check.check(root) == ["src/ARCHITECTURE.md: src/new.c is not listed"]
