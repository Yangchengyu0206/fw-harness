...
Writes the harness into a firmware repository: the entry documents, the state folders, the gate scripts, the git hooks, and the settings files that point at this marketplace. It then drafts `harness/architecture.json` from the real tree, asks you to confirm the modules, and generates an ARCHITECTURE.md for each of them.
